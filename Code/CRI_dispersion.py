...
def run_experiment(param_dict, output_fig_dir, output_data_dir):
    ...
    instab_sorted = np.take_along_axis(instab, sort_idx, axis=1)
    pspeed_sorted = np.take_along_axis(pspeed, sort_idx, axis=1)
    ...
    for i in range(disp_rel.shape[1]):
        if i < len(ls_list):
            ax[0].plot(k_dis, instab_sorted[:, i], color="k", linestyle=ls_list[i], 
    ...
    for i in range(disp_rel.shape[1]):
        if i < len(ls_list):
            ax[1].plot(k_dis, pspeed_sorted[:, i], color="k", linestyle=ls_list[i], linewidth=2.5)
